...
export class VirtualFileSystem {
  ...
  private updateChildrenPaths(node: FileNode): void {
    // Must update both the flat `files` Map (for O(1) lookups) and the
    // node.path property (used by the tree structure) to keep them in sync.
    if (node.type === "directory" && node.children) {
      for (const [_, child] of node.children) {
        const oldChildPath = child.path;
        child.path = node.path + "/" + child.name;

        // Re-key in files map so path-based lookups reflect the new location
